...
import io.github.interestinglab.waterdrop.utils.StructuredUtils
...
class Stdout extends BaseStructuredStreamingOutputIntra {
  ...
  override def prepare(spark: SparkSession): Unit = {
    ...
    val defaultConfig = ConfigFactory.parseMap(
      Map("streaming_output_mode" -> "Append",
  ...
  override def process(df: Dataset[Row]): DataStreamWriter[Row] = {
    ...
    var writer = df.writeStream.format("console").outputMode(config.getString("streaming_output_mode"))
